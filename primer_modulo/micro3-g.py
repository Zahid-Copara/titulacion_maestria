...
def dropbox_general(url, apk_name):
    tag = None
    aux_titulo = None
    try:
...
# This function clean title name of special char
def fix_title(title):
    dateNow = datetime.now()
    date_hour = dateNow.strftime('%m%y%H%M')
    titleFixed = title+'_'+date_hour
    return titleFixed
...
def apk_list(path):
    try:
        dataSetInput = pd.read_excel(path, engine="openpyxl")
    except Exception as e:
        reason = 'excel_file_not_available'
        logger.error("error on pandas library",
                     extra={'exception_message': str(e), 'reason': reason})
    else:
        return dataSetInput

# Save the data info into a csv file
def save_data(results):
    try:
        # Save the results into a Data Frame
        results_df = pd.DataFrame(results, columns=['nombre_app','categoria','numero_descargas','package_name','ubicacion','policy_privacy_url','policy_privacy_name'])
        # Save the results into a CSV
        results_df.to_excel(doc_file, index=False)
    except Exception as e:
        reason = 'error while saving excel'
        logger.error("save data into excel failed",
                     extra={'exception_message':str(e),'reason': reason})
                ...
                if state == True and code == 200:
                    ...
                    if downloadFlag == True:
                        ...
                        if csf_dropbox:
                            ...
                            dropbox_general(url, packageName)
                        doc_results.append((nombreApp, categoria, numeroDescargas, packageName, ubicacion, url, title))
